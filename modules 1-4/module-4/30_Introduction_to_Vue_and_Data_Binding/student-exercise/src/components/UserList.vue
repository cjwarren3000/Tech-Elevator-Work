  ...
  <table id="tblUsers">
    ...
    <tbody>
      <tr>
       ...
       <tr v-for="user of filteredList" v-bind:key="user.username" v-bind:class="{disabled: user.status === 'Disabled'}">
       ...
       let results = this.users
        if(this.filter.firstName){
          results = results.filter(item => {
            return item.firstName.toLowerCase().includes(this.filter.firstName.toLowerCase());
            })
       }

       if(this.filter.lastName){
          results = results.filter(item => {
            return item.lastName.toLowerCase().includes(this.filter.lastName.toLowerCase());
            })
       }

       if(this.filter.emailAddress){
          results = results.filter(item => {
            return item.emailAddress.toLowerCase().includes(this.filter.emailAddress.toLowerCase());
            })
       }

       if(this.filter.username){
          results = results.filter(item => {
            return item.username.toLowerCase().includes(this.filter.username.toLowerCase());
            })
       }
