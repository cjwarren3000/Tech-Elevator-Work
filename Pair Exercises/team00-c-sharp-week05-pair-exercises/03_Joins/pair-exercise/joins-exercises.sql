...
SELECT  AVG(city.population) AS AvPop FROM country
...
SELECT AVG(city.population) FROM country
JOIN countrylanguage ON country.code = countrylanguage.countrycode
JOIN city ON country.code = city.countrycode
WHERE countrylanguage.language = 'English'
AND isofficial = 1
...
SELECT country.continent, MAX(city.population) MaxCityPop FROM country
JOIN city ON country.code = city.countrycode
GROUP BY continent
