...
SELECT * FROM venue
JOIN category_venue ON venue.id = category_venue.venue_id JOIN category ON category_venue.category_id = category.id
ORDER BY venue.name

SELECT * FROM category c
JOIN category_venue cv ON cv.category_id = c.id

SELECT s.name, s.open_from, s.open_to, s.daily_rate, s.max_occupancy FROM space s 
JOIN venue v ON v.id = s.id
WHERE v.name = 'Painted Squirrel Club'


SELECT space.name, number_of_attendees, start_date, end_date, reserved_for, space.venue_id FROM reservation
JOIN space ON reservation.space_id = space.id
JOIN venue ON space.venue_id = venue.id
WHERE venue.name = 'Hidden Owl Eatery' 
AND ((start_date > '2020-06-22' AND start_date < '2020-06-25')
OR (end_date > '2020-06-22' AND end_date < '2020-06-25'))

SELECT TOP 5 space.id, venue_id, space.name, is_accessible, open_from, open_to, daily_rate, max_occupancy FROM space
JOIN venue ON space.venue_id = venue.id
WHERE venue.name = 'Hidden Owl Eatery'
